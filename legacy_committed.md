...
3. Build: title тЖТ outline тЖТ content тЖТ meta тЖТ alt texts тЖТ linking
...
5. **Offline mode:** If web access is unavailable, mark K11, L5, L6, L7, L8, T6, T7 as `тКШ [NO_WEB]` and exclude from denominator (see `shared/checklist.md тЖТ OFFLINE MODE`)
...
| C10 | Transition words | тЙе30% of sentences contain transitions | LOW |
...
| C14 | Content-to-ad ratio | Main content dominates above-fold | MEDIUM |
...
| L5 | Outbound link quality | DA 40+ or recognized authority | MEDIUM |
...
| T3 | SEO title length | 50тАУ60 characters (EN; per-language in `shared/readability-params.md`) | HIGH |
| T4 | Meta description length | 145тАУ158 characters (EN; per-language in `shared/readability-params.md`) | HIGH |
...
| A2 | E-E-A-T signals | Experience, Expertise, Authoritativeness, Trustworthiness | HIGH |
...
- Density: 0.8%тАУ1.5% (not zero, not stuffed)
...
### English
- Title: 50тАУ60 chars | Description: 145тАУ158 chars
- Flesch-Kincaid Grade 7тАУ9 target
- Power words: English list in `shared/power-words.md`

### Russian / ╨а╤Г╤Б╤Б╨║╨╕╨╣
- Title: 55тАУ70 chars (Cyrillic is wider) | Description: 140тАУ160 chars
- Avoid ╨║╨░╨╜╤Ж╨╡╨╗╤П╤А╨╕╨╖╨╝╤Л: ┬л╨╛╤Б╤Г╤Й╨╡╤Б╤В╨▓╨╗╤П╤В╤М┬╗, ┬л╤П╨▓╨╗╤П╤В╤М╤Б╤П┬╗, ┬л╨┤╨░╨╜╨╜╤Л╨╣┬╗
...
### Ukrainian / ╨г╨║╤А╨░╤Ч╨╜╤Б╤М╨║╨░
- Title: 55тАУ70 chars (Cyrillic) | Description: 140тАУ160 chars
...
| **Audit** | Cross-module header тЖТ Weighted Score + Grade тЖТ N/A тЖТ CRITICAL тЖТ HIGH тЖТ MEDIUM тЖТ LOW тЖТ PASSED тЖТ Impact тЖТ Top 5 Actions (full spec: `scenarios/audit-report.md`) |
| **Brief** | Cross-module header тЖТ Intent тЖТ Keywords тЖТ H1 Options тЖТ H2 Outline тЖТ Meta Drafts тЖТ Link Plan тЖТ Media Plan тЖТ Schema (full spec: `scenarios/content-brief.md`) |
...
1. **Quick-Fix detected** - "fix my [element]", "too long/short", "add keyword to", "remove" тЖТ Activate only the relevant module with fix instructions (route to `scenarios/quick-fix.md`)
...
**Offline mode detection:** Before audit operations, determine if web access is available (fetch tool, search capability). If NO web access тЖТ activate OFFLINE scoring (K11, L5, L6, L7, L8, T6, T7 тЖТ тКШ [NO_WEB], exclude from denominator). If YES тЖТ score all factors normally. See `shared/checklist.md` тЖТ OFFLINE MODE for full table.
...
```
rankwise/
тФЬтФАтФА SKILL.md                        тЖР This file - orchestrator
...
тФВ   тФФтФАтФА product-page.md             тЖР Product / e-commerce page optimization
тФФтФАтФА examples/
    тФЬтФАтФА before-after-article.md     тЖР Full article rewrite example
    тФЬтФАтФА meta-examples.md            тЖР Meta tag before/after examples
    тФФтФАтФА audit-example.md            тЖР Complete audit report example
```
